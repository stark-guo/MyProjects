...
<table width='100%' align='center' style='font-size: 13px;'
	...
	<tr>
		<td
			...
			总${pageModel.pageCount}页数&nbsp;&nbsp;总记录条数${pageModel.rowCount}
			<c:choose>
				<c:when test="${pageModel.pageNow==1}">
					<span class='disabled'>上一页</span>
					<c:forEach begin="1" end="${pageModel.pageCount}" var="i">
						<c:choose>
							<c:when test="${pageModel.pageNow==i}">
					...
					<span class='current'><a href='#'
						onclick="toPage(${pageModel.pageNow+1})">下一页</a></span>
				</c:when>
				<c:when test="${pageModel.pageNow==pageModel.pageCount}">
					<span class='current'><a href='#'
						onclick="toPage(${pageModel.pageNow-1})">上一页</a></span>
					<c:forEach begin="1" end="${pageModel.pageCount}" var="i">
						<c:choose>
							<c:when test="${pageModel.pageNow==i}">
								<span class='current'>${i}</span>
				...
				<c:otherwise>
					<span class='current'><a href='#' onclick="toPage(${pageModel.pageNow-1})" >上一页</a></span>
					<c:forEach begin="1" end="${pageModel.pageCount}" var="i">
						<c:choose>
							<c:when test="${pageModel.pageNow==i}">
								<span class='current'>${i}</span>
